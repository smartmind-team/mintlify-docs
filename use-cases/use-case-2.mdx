...
5. **Search by Image**:
    ...
    **Install Required Libraries**
    ...
    - This calls the `show_images` function with a single image URL of a coastal cliff. It will display the image inline in the notebook.
    ...
    query_result = client.query.execute(
        ...
        ORDER BY embedding <-> (SELECT embedding FROM calculated_embeddings)
        LIMIT 3
        """
    )
    check_query_result(query_result)
