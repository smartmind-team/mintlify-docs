...
3. **Classify Conversations and Aggregate Metrics**:
    ...
    - **Using Mistral LLM**:
      ...
      FROM 
          (
              -- Step 3: Generate categories for call transcripts using a Huggingface model
              SELECT 
                  "CallID",                  
                  thanosql.generate(
                      input := prompt,       
      ...
      JOIN 
          calls AS c ON t."CallID" = c."CallID" 
              -- Join the generated categories with the 'calls' table on 'CallID'
      GROUP BY 
          c."AgentID", t.category            
              -- Group the results by agent ID and category
      ORDER BY
          c."AgentID";                       
              -- Order the results by agent ID
      """)
    ...
    - **Using OpenAI GPT-4o**:
      ...
      GROUP BY 
          t.category;                       
              -- Group the results by generated category
      """)
      check_query_result(query_result)
      ```
      On execution, we get: 
